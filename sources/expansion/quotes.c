...
/*   Updated: 2025/12/22 11:42:03 by mgunter          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Note: Here we make bigger String than we need bcs we 
//don't use get_len_without_quotes(char *str) function. 
//But I don't think it is a problem.
// We will free it anyway.

static void	process_quote(char c, int *in_single, int *in_double)
{
	if (c == '\'' && !(*in_double))
		*in_single = !(*in_single);
	else if (c == '\"' && !(*in_single))
...
char	*remove_quotes(char *str)
{
	...
	while (str[i])
	{
		...
		{
			process_quote(str[i], &in_single, &in_double);
			i++;
		}
		else
			result[j++] = str[i++];
...
// Note: We can use it to have efficient memory,
	// but it is choice. Memory gonna be freed anyway.
