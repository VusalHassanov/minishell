...
// Parsing
int								parse_from_string(const char *arguments,
									t_shell *system);
...
void							assign_status(char character,
									t_parse_flags *status);
char							*dquote_handler(char *token_string,
									t_parse_flags *status);
char							*get_token_string(const char *arguments,
									t_parse_flags *status);
...
int								is_quote_literal(char character,
									t_parse_flags *status);

// Token Checker 2
int								is_quote(char character);
int								is_quote_matching(char character,
									t_parse_flags *status);
...
t_redir							**cleanup_redir_error(t_redir **redir,
									t_redir *new_redir);
...
int								get_input_heredoc_fd(char *delimitter,
									t_shell *system);
int								heredoc_gsignal_error(char *line, int write_fd,
									int read_fd);
char							*expand_if_needed(char *line, int expand_flag,
									t_shell *system);
...
char							*expand_exit_status(char *str, int pos,
									int exit_status);
void							copy_exit_status(char *dst, char *src,
									char *exit_str, int pos);
...
char							*expand_variable(char *str, int *i,
									t_shell *system, int in_heredoc);
int								get_var_len(char *str, int pos);
void							copy_with_var(char *dst, char *src, char *value,
									int pos);
